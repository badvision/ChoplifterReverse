...
def convert_all(chopgfx, addresses, head_on_idx):
    ...
    Convert all sprites.  Returns:
      headers  -- list of (w_cols, h, aux_ptr, hgr_addr, pixel_bytes)
    ...
    for addr in addresses:
        ...
        headers.append((w_cols, h, aux_ptr, addr, dhgr_pixels))

    return headers, aux_data
...
def emit_inc(headers, head_on_idx, inc_path):
    ...
    lines.append('; Story 5: DHGR sprite headers (4 bytes each) + address equates')
    lines.append('; Pixel data lives in AUX memory (CHOPAUX loaded to $6100 by loader).')
    lines.append('; blitImage reads pixel bytes via auxReadByte trampoline (no RAMRDAUX in code).')
    lines.append('')
    lines.append('; DHGR sprite header block -- placed at $AB1C')
    lines.append(f'.org ${DHGR_HEADER_BASE:04X}')
    lines.append('')

    for i, (w, h, aux_ptr, hgr_addr, pixels) in enumerate(headers):
        lo = aux_ptr & 0xFF
        hi = (aux_ptr >> 8) & 0xFF
        ...
        if i == head_on_idx:
            # Special label required by choplifter.s line 11874
            lines.append(f'chopperHeadOnDHGR0:')
        lines.append(f'{label_main}')
        lines.append(
            f'    .byte {w}, {h}, ${lo:02X}, ${hi:02X}'
            f'   {comment}  aux ptr = ${aux_ptr:04X}'
        )

    lines.append('')
    lines.append('; Address equates for pointer table (4 bytes per header):')
    for i in range(n):
        lines.append(f'dhgrSpriteAddr_{i:03d} = ${DHGR_HEADER_BASE:04X} + {i} * 4')
...
def emit_previews(headers, chopgfx, addresses, preview_dir):
    ...
    for i, (w_cols, h, aux_ptr, hgr_addr, dhgr_pixels) in enumerate(headers):
...
def validate(headers, aux_data, head_on_idx):
    n = len(headers)
    ...
    for i, (w, h, aux_ptr, _, _pixels) in enumerate(headers):
    ...
    w_ho, h_ho, _, addr_ho, _pix = headers[head_on_idx]
...
def main():
    ...
    # Convert sprites
    headers, aux_data = convert_all(chopgfx, addresses, head_on_idx)
    ...
    # Validate
    validate(headers, aux_data, head_on_idx)
